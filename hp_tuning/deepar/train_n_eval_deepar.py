...
def get_args():
    ...
    parser.add_argument('--num_layers', type=int, default=3, help='Number of the RNN layers.')
    parser.add_argument('--num_cells', type=int, default=10, help='Number of neurons per layer.')
    parser.add_argument('--cell_type', type=str, default='lstm', help='Type of the RNN cell; can be lstm or gru.')
    parser.add_argument('--drop_out', type=float, default=0.1, help='Dropout rate for the RNN layers.')
...
def setup_output_folder(args, timestamp) -> Path:
    output_folder = args.output_dir.joinpath(f'{timestamp}_{MODEL}_lr_{args.learning_rate}_epochs_{args.num_epochs}_bsize_{args.batch_size}_gradclip_{args.clip_grad}_nlayers_{args.num_layers}_ncells_{args.num_cells}_celltype_{args.cell_type}_dropout_{args.drop_out}')
...
def main():
    ...
    logger = setup_logger(file_name=f'{timestamp}_DEEPAR_HP', output_directory=output_folder, file_log_level='DEBUG', stream_log_level='INFO')
    ...
    output_df = pd.DataFrame({'model': MODEL, **vars(args), **agg_metrics, **perf_metrics}, index=[0])
