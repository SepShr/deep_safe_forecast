...
def get_args():
    ...
    parser.add_argument('--dropout', type=float, default=0.1, help='Dropout rate for the RNN layers.')
...
def setup_output_folder(args, timestamp) -> Path:
    output_folder = args.output_dir.joinpath(f'{timestamp}_{MODEL}_lr_{args.learning_rate}_epochs_{args.num_epochs}_bsize_{args.batch_size}_gradclip_{args.clip_grad}_nlayers_{args.num_layers}_ncells_{args.num_cells}_celltype_{args.cell_type}_dropout_{args.dropout}')
